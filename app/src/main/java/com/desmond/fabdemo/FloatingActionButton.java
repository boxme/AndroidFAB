...
public class FloatingActionButton extends ImageButton {
    ...
    void updateBackground() {
        ...
        // The background filled drawable
    ...
    Drawable getIconDrawable() {
        ...
        else if (mIcon != 0)       {return getResources().getDrawable(mIcon);}
    ...
    private Drawable createInnerStrokesDrawable(final int color, float strokeWidth) {
        ...
        shapeDrawable.setShaderFactory(new ShapeDrawable.ShaderFactory() {
            ...
            /** Called each time the drawable is resized (has a new width or height).
              * Its resize() method returns a corresponding shader, or null.
            * */
            @Override
            public Shader resize(int width, int height) {
                return new LinearGradient(width / 2, 0, width / 2, height,
                        new int[] {
                                topStrokeColor, topStrokeColorHalfTransparent, color,
    ...
    public void setSize(int size) {
        if (size != SIZE_MINI && size != SIZE_NORMAL) {
            throw new IllegalArgumentException("Use fab size contants");
        }
